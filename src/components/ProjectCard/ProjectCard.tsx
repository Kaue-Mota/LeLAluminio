...
export default function ProjectCard({ nome, src, variants, onClick }: ProjectCardProps) {
  const hasVariants = (variants?.length ?? 0) > 0;
  const extraCount = hasVariants && variants!.length > 4 ? variants!.length - 4 : 0;
  ...
  return (
    <div className="product-card group" onClick={onClick}>
      <div className="aspect-square overflow-hidden relative">
        ...
        {hasVariants && (
          <div className="absolute bottom-2 left-2 flex items-center gap-1 z-[1]">
